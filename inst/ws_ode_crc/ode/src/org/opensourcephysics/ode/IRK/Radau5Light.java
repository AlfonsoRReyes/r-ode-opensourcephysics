...
/**
 * Numerical solution of a stiffsystem of first order ordinary differential
...
public class Radau5Light implements ODESolver{
    ...
    protected double stepSize = 1e-6;
   ...
   */
    public Radau5Light(ODE ode) {
        ...
        intermediateStagesIncrement = new double[3][numEqn];
...
// TODO: uRound ??? to do something
        ...
        /**
         * Converts specific form of algebraic equation system of <code>IRKAlgebraicEquation</code>
         * to a general form of algebraic equation system G(x) = 0.
         * @param stageIncrement arguments array
         * @param residual function values array
         * @deprecated because the debugging
         */
        public void evaluate(double [][] stageIncrement, double [][] residual){
            double [][] a = new double [3][3];
            double [][] tmp_a = new double [3][3];
            a[0][0] = realEigenvalues[0];
            a[1][1] = complexEigenvalues[0];
            a[1][2] = -complexEigenvalues[1];
            a[2][1] = complexEigenvalues[1];
            a[2][2] = complexEigenvalues[0];
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    tmp_a[i][j] = 0;
                    for (int k = 0; k < 3; k++) {
                        tmp_a[i][j] += T[i][k]*a[k][j];
                    }
                }
            }
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    a[i][j] = 0;
                    for (int k = 0; k < 3; k++) {
                        a[i][j]  += tmp_a[i][k]*inverseT[k][j];
                    }
                }
            }

            double f[][] = new double[3][numEqn];
            for (int i = 0; i < 3; i++) {
                evaluateNonLinearComponent(stageIncrement[i], f[i]);
            }
            for (int k = 0; k < numEqn; k++) {
                for (int j = 0; j < 3; j++) {
                    for (int i = 0; i < 3; i++) {
                        residual[j][k] = a[j][i]*f[i][k];
                    }
                }
            }
            for (int k = 0; k < numEqn; k++) {
                for (int j = 0; j < 3; j++) {
                    residual[j][k] -= stageIncrement[j][k] / stepSize;
                }
            }
        ...
        /**
         * Gets the increment to the current intermedeate stages vectors array
         * as approximation to the solution of system of equations.
         * @return array of vectors of increment to the current intermedeate stages
         */
        ...
        public void evaluateNonLinearComponent(double[] freeVariable, double[] functionValue) {
          ...
          * esitmates for the moments of the model time concuiding the steps, even in case if
    ...
    /**
     * Calculates the incremet to the intermediate stages of difference
     * scheme as first approximation obtaied by one iteration of the
